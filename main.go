...
func loadingScreen(visible bool) {
	loadingElement := js.Global().Get("document").Call("getElementById", "loadingOverlay")
	if visible {
		loadingElement.Get("style").Set("display", "flex")
	} else {
		loadingElement.Get("style").Set("display", "none")
	}
}
